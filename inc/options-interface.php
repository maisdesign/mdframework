...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		// Wrap all options
		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) && ( $value['type'] != 'info')) {
		...
		if ( ( $value['type'] != 'heading' ) && ( $value['type'] != 'info') &&( $value['type'] != 'subinfo') ) {
		...
		case "info":
			...
			if ( isset($value['name']) ) {
				$output .= '<h4 class="heading postbox">' . esc_html( $value['name'] ) . '</h4>' . "\n";
			}
			if ( $value['desc'] ) {
